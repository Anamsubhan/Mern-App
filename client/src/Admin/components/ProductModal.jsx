...
function ProductModal() {
    ...
    const MultipleImageUpload = () => images?.map((val) => {
        const MultipleImageRef = ref(storage, `/images/products/${title}/${val.title}`);
    ...
    const AddProduct = (e) => {
        ...
        Promise.all(uploadImages)
            .then(() => {
                const storageRef = ref(storage, `/images/products/${title}/${thumb.name}`);
                uploadBytes(storageRef, thumb).then((snapshot) => {
                    getDownloadURL(snapshot.ref)
                        .then((url) => {
                            ...
                            axios.post('/api/addproduct', payload).then((json) => {
            ...
            <Modal show={show} onHide={handleClose} centered backdrop="static" size="lg">
                ...
                <Modal.Body>
                    ...
                    <form onSubmit={AddProduct}>
                        ...
                        <div className="row">
                            <div className="col">
                                <FloatingLabel controlId="title" label="Product Name" className="mb-3 text-secondary"                                >
                                    <Form.Control type="text" placeholder="Product Name" value={title} onChange={(e) => setTitle(e.target.value)} />
                                </FloatingLabel>
                            </div>
                            <div className="col">
                                <FloatingLabel controlId="price" label="Product Price (Rs)" className="mb-3 text-secondary"                                >
                        ...
                        <div className="mb-3">
                            ...
                            <input className="form-control" onChange={(e) => setThumb(e.target.files[0])} type="file" id="thumb" />
                        </div>
                            ...
                            <div className="col">
                                <Form.Group className="mb-3" >
                                    ...
                                    <FloatingLabel controlId="floatingSelectBrand" label="Select Brand">
                                        <Form.Select aria-label="Please Select a Brand" onChange={(e) => setBrandName(e.target.value)}>
                            ...
                            <div className="col">
                                <Form.Group className="mb-3" >
                                    <FloatingLabel controlId="selectCategory" label="Select Category">
                                        <Form.Select aria-label="Please Select a Category" onChange={(e) => setCatName(e.target.value)}>
                                            <option>Please Select a Category</option>
                                            {
                                                CategoryVal.map((val, key) => <option key={key} value={val.catname}>{val.catname}</option>)
                                            }
                                        </Form.Select>
                                    </FloatingLabel>
                                </Form.Group>
